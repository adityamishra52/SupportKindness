...
export default function AnimalHelpPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-10 px-6 py-12 lg:px-10">
      ...
      <Section title="Animal Gallery" subtitle="Activity moments from care missions."><div className="columns-1 gap-3 space-y-3 sm:columns-2 lg:columns-3">{(animalImages.length ? animalImages : gallery.slice(0, 6)).map((item) => <motion.button key={item._id} type="button" onClick={() => setSelected(item)} whileHover={{ scale: 1.01 }} className="group relative w-full overflow-hidden rounded-2xl"><img src={asset(item.imageUrl)} alt={item.title} loading="lazy" className="w-full object-cover transition duration-500 group-hover:scale-110" /></motion.button>)}</div></Section>
      ...
      <AnimatePresence>{selected && <motion.button type="button" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelected(null)} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"><img src={asset(selected.imageUrl)} alt={selected.title} className="max-h-[90vh] rounded-2xl" /></motion.button>}</AnimatePresence>
    </div>
  );
}