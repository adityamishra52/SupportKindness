...
export default function TreePlantationPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-10 px-6 py-12 lg:px-10">
      ...
      <Section title="Gallery" subtitle="Visual highlights from drives."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{(treeGallery.length ? treeGallery : gallery.slice(0, 6)).map((item) => <img key={item._id} src={asset(item.imageUrl)} alt={item.title} className="h-52 w-full rounded-xl object-cover" loading="lazy" />)}</div></Section>
